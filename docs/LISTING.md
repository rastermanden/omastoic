...
a freshly composed canvas in that file and swaps it between effects. Switching
lives where Omarchy already put it: **Style → Screensaver**, next to Edit
Text, Set From Image and Restore Default. Omarchy's own branding commands
win — set the art some other way and the Stoics stand aside.
...
- **Style → Screensaver → Choose** opens a grid of preview tiles.
- Your own art in `~/.config/omastoic/screensavers/<Name>.txt` joins the grid.
...
The Stoics on your Omarchy screensaver — portraits, cited quotes, and a
picker next to the ones Omarchy already ships.
...
- It adds two rows to `~/.config/omarchy/extensions/omarchy-menu.jsonc`
